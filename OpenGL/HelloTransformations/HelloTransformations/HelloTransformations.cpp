...
HelloTransformations::HelloTransformations(QWidget *parent)
    : QOpenGLWidget(parent)
{
    //ui.setupUi(this);
}

void HelloTransformations::initializeGL() {
	/// 着色器部分
...
void HelloTransformations::resizeGL(int w, int h) {}

void HelloTransformations::paintGL() {
	...
	ourShader->use();
	core->glBindVertexArray(VAO);
	QMatrix4x4 mat;//默认单位矩阵
	//mat.scale(2);
	//mat.rotate(20.0f, QVector3D(0.0f, 0.0f, 1.0f));
	//mat.translate(0.3, 0.1, 0.0);
